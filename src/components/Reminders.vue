<template>
  <div>
    ...
    <v-btn @click="scheduleEvents">Schedule</v-btn>
    {{wkEvents}}
  </div>
</template>
...
export default {
    ...
    async scheduleEvents() {
        ...
        if (this.wkEvents.length !== 0) {
            console.log('already scheduled once')
            return
        }
            ...
            for (var id = 0; id < this.wkEvents[idx].length - 1; id++) {
                ...
                .then(async (res) => {
                    ...
                    await this.$store.getters.gapiCalendarSwitch.insert({
                        ...
                        start  : {
                                dateTime: nextEvent.date + "T" + this.getTime( nextEvent.startTime, res.data.plan.itineraries[0].duration ) + "+08:00" ,
                ...
                .catch(err => {
                    alert('you lousy bastard, fucked up')
                })
