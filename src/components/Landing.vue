  ...
  background-attachment: fixed; background-position: center">
      <v-container fill-height>
        <v-layout align-center>
          <v-flex offset-xl1 xs12 sm10 md8 xl6 style="background-color: rgba(0, 0, 0, 0.4); border-radius: 25px" class="white--text px-4 py-2">
            <h3 style="font-weight: bold" class="display-3">Welcome to HowGo</h3>
            <div class="subheading">
              A personal travel assistant which helps users get to their planned locations in a time or cost-efficient way.
            </div>
            <div class="subheading">
              Now, there is no need to constantly search for ways to get to places as things would be automated.
            </div>
      ...
      <v-container id="routing">
        <v-responsive class="subheading mb-5">
          <v-container class="pa-0" fill-height>
            <v-layout align-center>
              <v-flex>
                <h3 class="display-2">Have a Go</h3>
                <v-divider class="my-3"></v-divider>
                <span class="subheading">
                  HowGo utilises the OneMap API Routing Service to get to your destinations. Simply enter two locations to view your route options.
                </span>
              </v-flex>
            </v-layout>
          </v-container>
...
export default {
  ...
  methods: {
    fillFrom(place, response) {
      ...
      eventInfo['latlng'] = response.geometry.location.lat() + ',' + response.geometry.location.lng()
      this.from = eventInfo
      if (this.to) {
        this.getRoute()
      }
      ...
      navigator.geolocation.getCurrentPosition(async (position) => {
        ...
        .then(res => {
          ...
          if (res.data.GeocodeInfo[0].BUILDINGNAME) {
            this.fromStr = capitalize(res.data.GeocodeInfo[0].BUILDINGNAME) + ', ' + capitalize(res.data.GeocodeInfo[0].ROAD)
          } else {
